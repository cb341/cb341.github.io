...
$$
\forall a \in \mathbb{N} : a + 0 \stackrel{\mathrm{def}}{=} a
$$

**Succ:**

$$
(a, d : \mathbb{N}) : a + succ(d) \stackrel{\mathrm{def}}{=} succ (a + d)
$$
...
$$
  ...
  \begin{array}{l}
  a. 1 \stackrel{\mathrm{def}}{=} succ(0) \\
  b. 2 \stackrel{\mathrm{def}}{=} succ(1) \\
  c. 3 \stackrel{\mathrm{def}}{=} succ(2) \\
  \end{array}  \\
  
  \begin{array}{l|l}
  1 + 2                                               & \text{Given}         \\
  1 + \operatorname{succ}(1)                          & \mathrm{i.}          \\
  \operatorname{succ}(1 + 1)                          & \mathrm{i.}          \\
  \operatorname{succ}(1 + \operatorname{succ}(0))     & \mathrm{i.}          \\
  \operatorname{succ}(\operatorname{succ}(1 + 0))     & \mathrm{i.}          \\
  \operatorname{succ}(\operatorname{succ}(1))         & \mathrm{i.}          \\
  \operatorname{succ}(2)                              & \mathrm{i.}          \\
  3                                                   & QED. \Box \\
  \end{array}
$$
...
\hline
1. \ 1 + 2 = 3             & 1. \ \text{Given} \\
2. \ 1 + succ(1) = succ(2) & 2. \ \text{Definition of congruent angles} \\
3. \ succ(1 + 1) = succ(2) & 3. \ \text{Given} \\
4. \                       & 4. \ \text{Transitive Property of Equality} \\
\end{array}
$$

We don't proove that $a + 0 = a$, we define it.

### Inequalities?

What we can do next is define inequalities.

$$
(a,b : \mathbb{N} ) a ≤ b \stackrel{\mathrm{def}}{\implies} ∃ (c : ℕ), b = a + c
$$
